...
def p_new_i_j_k(i, j, k, R0, s_new, h, R, sigma_r):
    return max(0, min(1, (
                mu_s(r_i_k(i, k, R, sigma_r), s_new[i][j]) * h + s_new[i][j] - s_new[i + 1][j_d_new_i_j_k(i, j, k, R0, s_new, h)]) / (
...
def m_i_jd_ku(i, j, k,R0, s_new, h, R, sigma_r):
    return (s_new[i + 1][j_d_new_i_j_k(i, j, k, R0, s_new, h)] - s_new[i][j]) * (
                r_i_k(i + 1, k_u_new_i_k(i, k, R0, h), R, sigma_r) - r_i_k(i, k, R, sigma_r))


def m_i_ju_kd(i, j, k,R0, s_new, h, R, sigma_r):
    return (s_new[i + 1][j_u_new_i_j_k(i, j, k, R0, s_new, h)] - s_new[i][j]) * (
                r_i_k(i + 1, k_d_new_i_k(i, k, R0, h), R, sigma_r) - r_i_k(i, k, R, sigma_r))
...
def transition_probabilities(i, j, k,R0, s_new, h, R, sigma_r):
    ...
    b = np.array(
        [p_new_i_j_k(i, j, k, R0, s_new, h, R, sigma_r), p_new_i_k(i, k, R0, h), 1,
    ...
    for i in range(N - 1, -1, -1):
        ...
        for j in range(0, i + 1):
            ...
            for k in range(0, i + 1):
                ...
                # print(j_u_new_i_j_k(i, j, k, R0, s_new, h), k_u_new_i_k(i, k, R0, h))
...
def simulation_MC(N, Y, R, sigma_r, h):
    s = S_0
    r = r_0
    i = 0
    j = 0
...
def MC_tree(nb_simul, N, Y, R, sigma_r, h):
    ...
    for i in range(nb_simul):
        r, s = simulation_MC(N, Y, R, sigma_r, h)
        tab_r.append(r)
        ...
        for valeur3 in tab_N:
            ...
            ecrivainCSV.writerow(
                ["T = " + str(valeur1) + "; sigma_R = " + str(valeur2) + "; N = " + str(valeur3), str(v[0][0][0]),
                 str(update_v_euro([initialize_v_euro(N, Y, R)],Y, R, sigma_r, h, N)[0][0][0]), str(update_v_new([initialize_v_new(N, s_new)], R0, s_new, h, N, R, sigma_r)[0][0][0]),
                 str(update_v_new_euro([initialize_v_new_euro(N, s_new)], R0, s_new, h, N, R, sigma_r)[0][0][0]), str(s_MC), str(s_MC_tree)])
